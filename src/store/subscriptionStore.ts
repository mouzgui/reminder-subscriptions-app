...
import {
  scheduleAllRemindersForSubscription,
  cancelAllRemindersForSubscription,
  ...
  persist(
    (set, get) => ({
      ...
      deleteSubscription: async (id: string) => {
        set({ isLoading: true, error: null });
        try {
          // Check if it's a local/demo subscription first
          if (id.startsWith("local-") || id.startsWith("demo-")) {
            get().deleteLocalSubscription(id);
            set({ isLoading: false });
            return;
          }

          // Optimistic update for cloud subscriptions
      ...
      getExpiringSoonSubscriptions: (days: number = 7) => {
        ...
        return get().subscriptions.filter((sub) => {
          ...
          return (
            sub.is_active && renewalDate >= now && renewalDate <= futureDate
          );
        });
      },
      ...
      calculateBurnRate: (currency: CurrencyCode) => {
        return get()
          .subscriptions.filter(
            (sub) => sub.is_active && sub.currency === currency
          )
